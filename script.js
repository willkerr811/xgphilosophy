document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateTable() {
        ...
        uniquePoints.forEach(point => {
            ...
            row.innerHTML = `
                <td>${point}</td>
                <td>${teamsWithExpectedPoint.map(team => `<img src="${team.logo}" alt="${team.team} Logo" style="width: 30px;;">`).join('<br>')}</td>
                <td>${teamsWithActualPoint.map(team => `<img src="${team.logo}" alt="${team.team} Logo" style="width: 30px;">`).join('<br>')}</td>
            `;
