document.addEventListener('DOMContentLoaded', function () {
        ...
        uniquePoints.forEach((point, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="expected-column">
                    <div class="logo-container" id="expected-${index}" ondrop="drop(event)" ondragover="allowDrop(event)">
                        ${createLogosHTML(teamsWithExpectedPoint)}
                    </div>
                </td>
                <td class="actual-column">
                    <div class="logo-container" id="actual-${index}" ondrop="drop(event)" ondragover="allowDrop(event)">
    ...
    function createLogosHTML(teams, column) {
        return teams.map((team, teamIndex) => `<img src="${team.logo}" alt="${team.team} Logo" class="logo ${column}-${team.team}-${teamIndex}" 
            id="${team.team}-${teamIndex}" draggable="true" ondragstart="drag(event)">`).join(' ');
    }
...
function drag(event) {
    const teamName = event.target.alt.split(' Logo')[0]; // Extract team name from alt attribute
    const column = event.target.parentElement.classList.contains('expected-column') ? 'expected' : 'actual';
    event.dataTransfer.setData('text', `${teamName}-${column}`);
}
...
function drop(event) {
    ...
    const [teamName, column] = data.split('-');
    const draggedElement = document.getElementById(`${teamName}-${column}`);
    ...
    if (draggedElement) {
        ...
        // Append the dragged element to the body
